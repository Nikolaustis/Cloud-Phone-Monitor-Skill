...
const purchaseModePlatforms = new Set(["UgPhone", "VSPhone"]);
...
// Keep long daily histories readable: charts use a small, stable set of evenly
// distributed date labels while the tooltip still exposes the full ISO date.
function compactDateLabel(value) {
  const text = String(value || "");
  return /^\d{4}-\d{2}-\d{2}$/.test(text) ? text.slice(5) : text;
}

function buildChartDateTicks(rows, maxTicks = 8) {
  const dates = [...new Set(stableArray(rows)
  ...
  for (let index = 0; index < maxTicks; index += 1) {
    const position = Math.round((index * (dates.length - 1)) / (maxTicks - 1));
    selected.add(dates[position]);
  }
...
function SelectFilter({ label, value, onChange, children, className = "" }) {
  return (
    <label className={`grid gap-1 text-xs font-semibold text-muted ${className}`}>
      {label}
...
function SearchFilter({ label, value, onChange, placeholder = "搜索", className = "" }) {
  return (
    <label className={`grid gap-1 text-xs font-semibold text-muted md:col-span-2 ${className}`}>
      {label}
...
function AppShell({ page, setPage, lang, setLang, onReload, isReloading, dataMeta, children }) {
  return (
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-line bg-white/90 px-4 backdrop-blur lg:px-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              {dataMeta?.current_price_authority ? (
                <div className="mt-0.5 text-[11px] text-muted" title={`数据版本：${dataMeta.current_price_data_revision || "-"}`}>
                  当前价格：本次 products.csv · {dataMeta.last_run_date || "日期未知"} · 版本 {String(dataMeta.current_price_data_revision || "-").slice(0, 8)}
                </div>
              ) : null}
            </div>
          </div>
...
// Chart data always keeps the true price on the plotted Y coordinate. When
// several product lines have exactly the same price, we do not move one of
// them up/down: that would incorrectly imply a price difference. Instead we
// keep the genuine shared level and add colour-segment overlays for the lines
// that would otherwise be fully hidden below the last-rendered stroke.
function visualChartDataKey(dataKey) {
  return `${String(dataKey)}__visual`;
}

function logicalChartDataKey(dataKey) {
  return String(dataKey || "").replace(/__visual$/, "");
}
...
function isOverlapPatternDataKey(value) {
  const text = String(value || "");
  return text.startsWith("__overlap_segment__") || text.startsWith("__overlap_pattern__");
}

const NEAR_OVERLAP_VALUE_EPSILON = 0.08;

function buildOverlapGradientStops(colors) {
  const palette = [...new Set(stableArray(colors).filter(Boolean))];
  if (palette.length <= 1) {
    const color = palette[0] || "#64748b";
    return [
      { offset: "0%", color },
      { offset: "100%", color },
    ];
  }
  // This is a smooth colour weave, not a fixed dash pattern. The stops are
  // attached to one detected real overlap segment only, so they begin/end
  // exactly where the source product lines begin/end sharing a value.
  const transitions = Math.max(6, palette.length * 4);
  return Array.from({ length: transitions + 1 }, (_, index) => ({
    offset: `${((index / transitions) * 100).toFixed(2)}%`,
    color: palette[index % palette.length],
  }));
}

// Chart data always keeps the true price on the plotted Y coordinate. When
// product lines share one exact price for a continuous run of dates, we build
// one overlay series for that *actual run* only. There is no global pre-drawn
// dash pattern: if the values cease to coincide, the colour weave ceases too.
function buildOverlapPatternRenderModel(rows, logicalKeys, namespace = "overlap", options = {}) {
  const keys = [...new Set(stableArray(logicalKeys).map((key) => String(key)).filter(Boolean))];
  // Figure two is plotted in real money units, where a few cents may already
  // cause line strokes to visually overlap. Figure one is plotted in index
  // units, so the same 0.08 threshold was effectively invisible there.
  const nearOverlapValueEpsilon = Number.isFinite(Number(options.nearValueEpsilon))
    ? Number(options.nearValueEpsilon)
    : (namespace === "index" ? 2.0 : NEAR_OVERLAP_VALUE_EPSILON);
  const metadataSuffixes = ["source", "carried_from_date", "source_collection_date", "label", "count", "inputs"];

  const chartRows = stableArray(rows).map((row) => {
    const next = { ...row };
    keys.forEach((key) => {
      // A missing observation must remain missing.  Number(null) is 0 in
      // JavaScript; coercing it here created a false line segment from the
      // chart baseline to the first real observation.  Preserve null/blank
      // values so Recharts starts the line on the first valid date instead.
      const rawValue = row?.[key];
      if (rawValue === null || rawValue === undefined || rawValue === "") return;
      const value = Number(rawValue);
      if (!Number.isFinite(value)) return;
      const visualKey = visualChartDataKey(key);
      next[visualKey] = value;
      next[`${visualKey}__actual_value`] = value;
      metadataSuffixes.forEach((suffix) => {
        const sourceKey = `${key}__${suffix}`;
        if (row[sourceKey] !== undefined) next[`${visualKey}__${suffix}`] = row[sourceKey];
      });
    });
    return next;
  });

  const groupsByRow = chartRows.map((row) => {
    const values = new Map();
    keys.forEach((key) => {
      const value = Number(row?.[key]);
      if (!Number.isFinite(value)) return;
      const valueKey = value.toFixed(9);
      const group = values.get(valueKey) || [];
      group.push(key);
      values.set(valueKey, group);
    });
    const groups = new Map();
    values.forEach((members) => {
      if (members.length < 2) return;
      const ordered = keys.filter((key) => members.includes(key));
      groups.set(ordered.join("||"), ordered);
    });
    return groups;
  });

  // Group membership (not a hard-coded pattern) determines the overlap. A
  // group may move together in price and still remains one genuine overlap run.
  const runsBySignature = new Map();
  groupsByRow.forEach((groups, rowIndex) => {
    groups.forEach((sourceKeys, signature) => {
      const runs = runsBySignature.get(signature) || [];
      const latest = runs[runs.length - 1];
      if (latest && latest.endIndex === rowIndex - 1) {
        latest.endIndex = rowIndex;
      } else {
        runs.push({ startIndex: rowIndex, endIndex: rowIndex, sourceKeys });
      }
  ...
  runsBySignature.forEach((runs, signature) => {
    runs.forEach((run) => {
      // A line segment needs at least two adjacent observations. One-day
      // coincidences are still truthful in the tooltip but do not need a line.
      if (run.endIndex <= run.startIndex) return;
      const hash = shortStableHash(`${namespace}::${signature}::${run.startIndex}-${run.endIndex}`);
      const dataKey = `__overlap_segment__${namespace}__${hash}`;
      for (let index = run.startIndex; index <= run.endIndex; index += 1) {
        const sharedValue = Number(chartRows[index]?.[run.sourceKeys[0]]);
        if (Number.isFinite(sharedValue)) chartRows[index][dataKey] = sharedValue;
      }
      overlapPatterns.push({
        dataKey,
        gradientId: `overlap-gradient-${namespace}-${hash}`,
  ...
  const proximityRunsBySignature = new Map();
  chartRows.forEach((row, rowIndex) => {
    ...
    for (let index = 0; index < ordered.length - 1; index += 1) {
      ...
      const delta = upper.value - lower.value;
      if (!(delta > 1e-9 && delta <= nearOverlapValueEpsilon)) continue;
      const sourceKeys = [upper.key, lower.key];
      const rendererKey = [...sourceKeys].sort((a, b) => keys.indexOf(a) - keys.indexOf(b)).at(-1);
      const signature = `${sourceKeys.join('~~')}::${rendererKey}`;
      const runs = proximityRunsBySignature.get(signature) || [];
      const latest = runs[runs.length - 1];
      if (latest && latest.endIndex === rowIndex - 1) {
        latest.endIndex = rowIndex;
      } else {
      ...
      proximityRunsBySignature.set(signature, runs);
    }
  });

  const proximityPatterns = [];
  proximityRunsBySignature.forEach((runs, signature) => {
    runs.forEach((run) => {
      if (run.endIndex <= run.startIndex) return;
      const hash = shortStableHash(`${namespace}::near::${signature}::${run.startIndex}-${run.endIndex}`);
      proximityPatterns.push({
        dataKey: `__overlap_pattern__${namespace}__${hash}`,
        gradientId: `overlap-near-gradient-${namespace}-${hash}`,
        sourceKeys: run.sourceKeys,
        upperKey: run.upperKey,
        lowerKey: run.lowerKey,
        rendererKey: run.rendererKey,
        startIndex: run.startIndex,
        endIndex: run.endIndex,
        kind: 'near',
      });
  ...
  return { rows: chartRows, overlapPatterns, proximityPatterns };
}

// Custom Recharts line shapes bypass the library's built-in path animation.
// Reveal the rendered SVG through a moving clip rectangle instead: every line
// (including a real overlap gradient) is drawn left-to-right on its actual
// coordinates, without inventing values or exposing a fixed pre-drawn pattern.
function useSvgLineReveal(revealKey, duration = 1100) {
  ...
  useEffect(() => {
    ...
    const animate = (now) => {
      ...
      // ease-out keeps the first part of the left-to-right sweep legible.
      const eased = raw >= 1 ? 1 : 1 - Math.pow(1 - raw, 3);
      setProgress(eased);
      if (raw < 1) frameId = window.requestAnimationFrame(animate);
    };
...
// For a true equal-price run, all participating lines retain the exact same
// Y coordinate. The last line in the render order owns a native SVG overlay
// whose gradient is calculated from that real overlap run; the other members
// omit only the covered intervals. This prevents one colour from masking the
// other without manufacturing a price difference.
function OverlapAwareLineShape({
  points = [],
  logicalKey,
  patterns = [],
  proximityPatterns = [],
  ...
  const revealProgress = useSvgLineReveal(revealKey);
  if (!Array.isArray(points) || points.length < 2) return null;

  const drawablePoints = points.filter((point) => Number.isFinite(point?.x) && Number.isFinite(point?.y));
  if (drawablePoints.length < 2) return null;
  const minX = Math.min(...drawablePoints.map((point) => point.x));
  const maxX = Math.max(...drawablePoints.map((point) => point.x));
  const revealClipId = `line-reveal-${shortStableHash(`${logicalKey}|${revealKey}|${minX}|${maxX}`)}`;
  const revealWidth = Math.max(0, (maxX - minX + 10) * revealProgress);

  if (pointRegistryRef?.current) pointRegistryRef.current.set(logicalKey, points);

  const relevantPatterns = stableArray(patterns).filter((pattern) => pattern.sourceKeys.includes(logicalKey));
  const relevantProximityPatterns = stableArray(proximityPatterns).filter((pattern) => pattern.rendererKey === logicalKey);
  const coveredIntervals = new Set();
  relevantPatterns.forEach((pattern) => {
    for (let index = pattern.startIndex; index < pattern.endIndex; index += 1) coveredIntervals.add(index);
  });
  ...
  for (let index = 0; index < points.length - 1; index += 1) {
    ...
    if (coveredIntervals.has(index)) continue;
    normalSegments.push(`M${left.x.toFixed(2)},${left.y.toFixed(2)} L${right.x.toFixed(2)},${right.y.toFixed(2)}`);
  }

  return (
    <g clipPath={clipPath} className="recharts-line-curve">
      <defs>
        <clipPath id={revealClipId}>
          <rect x={minX - 5} y={-100000} width={revealWidth} height={200000} />
        </clipPath>
      </defs>
      <g clipPath={`url(#${revealClipId})`}>
      {normalSegments.map((d, index) => (
        <path
          key={`normal-${logicalKey}-${index}`}
          d={d}
          fill="none"
          stroke={stroke}
      ...
      {relevantPatterns.map((pattern) => {
        // Only the last participant draws the shared run. That keeps exactly
        // one path on the genuine Y coordinate, rather than stacking lines or
        // offsetting values. The gradient is defined *inside* this SVG shape
        // using userSpaceOnUse coordinates calculated from the actual rendered
        // points. This avoids Recharts/SVG fallback to a single line colour.
        const rendererKey = pattern.sourceKeys[pattern.sourceKeys.length - 1];
        if (rendererKey !== logicalKey) return null;

        const pathPoints = points.slice(pattern.startIndex, pattern.endIndex + 1)
          .filter((point) => Number.isFinite(point?.x) && Number.isFinite(point?.y));
        if (pathPoints.length < 2) return null;

        const first = pathPoints[0];
        const last = pathPoints[pathPoints.length - 1];
        const d = pathPoints.map((point, index) => `${index === 0 ? "M" : "L"}${point.x.toFixed(2)},${point.y.toFixed(2)}`).join(" ");
        const focused = Boolean(focusedDataKey && pattern.sourceKeys.includes(focusedDataKey));
        const dimmed = Boolean(focusedDataKey && !focused);
        const palette = pattern.sourceKeys.map((key) => colorForKey(key)).filter(Boolean);
        const localGradientId = `overlap-live-${shortStableHash(`${pattern.dataKey}-${first.x}-${first.y}-${last.x}-${last.y}`)}`;
        const horizontalSpan = Math.abs(last.x - first.x);
        const verticalSpan = Math.abs(last.y - first.y);
        const gradientX2 = horizontalSpan >= verticalSpan ? last.x : first.x;
        const gradientY2 = horizontalSpan >= verticalSpan ? first.y : last.y;

        return (
          <Fragment key={`overlap-${pattern.dataKey}-${focusedDataKey || "all"}`}>
            {!focused ? (
              <defs>
                <linearGradient
                  id={localGradientId}
                  ...
                  y2={gradientY2}
                >
                  {buildOverlapGradientStops(palette).map((stop, index) => (
                    <stop
                      key={`${localGradientId}-${index}`}
                      offset={stop.offset}
            ...
            {/* A thin first-colour underlay ensures the actual run remains
                visible even if a browser delays gradient paint during the
                left-to-right line animation. The gradient path immediately
                covers it after paint. */}
            <path
              d={d}
              fill="none"
              stroke={focused ? colorForKey(focusedDataKey) : (palette[0] || stroke)}
              strokeOpacity={dimmed ? 0.12 : 1}
              strokeWidth={focused ? Math.max(Number(strokeWidth) + 1.4, 6.0) : Math.max(Number(strokeWidth) + 1.1, 5.2)}
              strokeLinecap="round"
              strokeLinejoin="round"
            />
            {!focused ? (
              <path
                d={d}
                fill="none"
                stroke={`url(#${localGradientId})`}
                strokeOpacity={dimmed ? 0.12 : 1}
                strokeWidth={Math.max(Number(strokeWidth) + 1.1, 5.2)}
                strokeLinecap="round"
                strokeLinejoin="round"
              />
            ) : null}
      ...
      {relevantProximityPatterns.map((pattern) => {
        const upperPoints = pointRegistryRef?.current?.get(pattern.upperKey);
        const lowerPoints = pointRegistryRef?.current?.get(pattern.lowerKey);
        if (!Array.isArray(upperPoints) || !Array.isArray(lowerPoints)) return null;
        const pairedPoints = [];
        for (let index = pattern.startIndex; index <= pattern.endIndex; index += 1) {
          const upper = upperPoints[index];
          const lower = lowerPoints[index];
          if (!Number.isFinite(upper?.x) || !Number.isFinite(upper?.y) || !Number.isFinite(lower?.x) || !Number.isFinite(lower?.y)) continue;
          pairedPoints.push({
            x: (upper.x + lower.x) / 2,
            y: (upper.y + lower.y) / 2,
            upperY: upper.y,
        ...
        if (pairedPoints.length < 2) return null;
        const first = pairedPoints[0];
        const last = pairedPoints[pairedPoints.length - 1];
        const d = pairedPoints.map((point, index) => `${index === 0 ? "M" : "L"}${point.x.toFixed(2)},${point.y.toFixed(2)}`).join(" ");
        const upperColor = colorForKey(pattern.upperKey);
        const lowerColor = colorForKey(pattern.lowerKey);
        const focused = Boolean(focusedDataKey && pattern.sourceKeys.includes(focusedDataKey));
        const dimmed = Boolean(focusedDataKey && !focused);
        const localGradientId = `overlap-near-live-${shortStableHash(`${pattern.dataKey}-${first.x}-${first.y}-${last.x}-${last.y}`)}`;
        const averageGap = pairedPoints.reduce((sum, point) => sum + Math.abs(point.upperY - point.lowerY), 0) / pairedPoints.length;
        const topY = pairedPoints.reduce((sum, point) => sum + point.upperY, 0) / pairedPoints.length;
        const bottomY = pairedPoints.reduce((sum, point) => sum + point.lowerY, 0) / pairedPoints.length;
        const bandWidth = Math.max(Number(strokeWidth) + 1.4, averageGap + Number(strokeWidth) + 1.2);
        return (
          <Fragment key={`proximity-${pattern.dataKey}-${focusedDataKey || "all"}`}>
            {!focused ? (
              <defs>
                <linearGradient
                  id={localGradientId}
                  ...
                  y2={bottomY}
                >
                  <stop offset="0%" stopColor={upperColor} stopOpacity={0.96} />
                  <stop offset="50%" stopColor={upperColor} stopOpacity={0.78} />
                  <stop offset="50%" stopColor={lowerColor} stopOpacity={0.78} />
                  <stop offset="100%" stopColor={lowerColor} stopOpacity={0.96} />
                </linearGradient>
              </defs>
            ) : null}
            <path
              d={d}
              fill="none"
              stroke={focused ? colorForKey(focusedDataKey) : `url(#${localGradientId})`}
              strokeOpacity={dimmed ? 0.12 : 0.98}
              strokeWidth={focused ? Math.max(bandWidth + 0.2, Number(strokeWidth) + 2.0) : bandWidth}
              strokeLinecap="round"
              strokeLinejoin="round"
            />
          </Fragment>
...
function productLineGroupKey(series) {
  ...
  return [
    ...
    android,
    seriesPurchaseMode(series),
  ].join("||");
}

// Android 展开模式下，历史数据可能同时存在“Android 已识别”和“Android 未识别”
// 的同一产品快照（通常只是在旧采集记录中缺少 Android 字段）。两者不是两款
// 独立产品；若它们的平台、套餐、购买时长和非 Android 配置一致，应优先保留
// 已识别 Android 的真实产品线，避免“UgPhone UVIP 30天”和“UVIP Android 10 30天”
// 同时出现在同一个配对组并造成默认选线/缺失感知错误。
function productLineFingerprintWithoutAndroid(series) {
  const platform = String(series?.platform || "").trim();
  const product = cleanProductModelForProductLine(series?.product_model || series?.ug_product_model || "");
  const duration = String(series?.duration_bucket ?? "");
  const config = String(series?.config || "")
    .replace(/\/?\s*Android\s+[^/]+(?=\s*\/|$)/gi, "")
    .replace(/\s+/g, " ")
    .trim()
    .toLowerCase();
  return [platform, product, duration, config, seriesPurchaseMode(series)].join("||");
}

function platformSupportsPurchaseMode(platform) {
  return purchaseModePlatforms.has(String(platform || "").trim());
}

function seriesPurchaseMode(series) {
  if (!platformSupportsPurchaseMode(series?.platform)) return "standard";
  const raw = String(series?.purchase_mode || series?.purchaseMode || "subscription").trim().toLowerCase().replace(/-/g, "_");
  return ["non_subscription", "nonsubscription", "one_time", "onetime", "single_purchase", "off", "false", "0"].includes(raw)
    ? "non_subscription"
    : "subscription";
}

function trendPointFreshness(point) {
  if (!point || point.price === null || point.price === undefined || point.price === "") return 0;
  const source = String(point.price_source || "").toLowerCase();
  if (source === "missing") return 0;
  if (source === "carry_forward") return 1;
  if (source === "baseline_fallback") return 2;
  return 3;
}

function mergeTrendPointLists(primaryPoints, fallbackPoints) {
  const byDate = new Map();
  stableArray(fallbackPoints).forEach((point) => {
    if (point?.date) byDate.set(String(point.date), point);
  });
  stableArray(primaryPoints).forEach((point) => {
    if (!point?.date) return;
    const date = String(point.date);
    const existing = byDate.get(date);
    if (!existing || trendPointFreshness(point) >= trendPointFreshness(existing)) byDate.set(date, point);
  });
  return [...byDate.values()].sort((a, b) => String(a?.date || "").localeCompare(String(b?.date || "")));
}

function mergeRegionalTrendMaps(primaryMap, fallbackMap) {
  const regions = new Set([...Object.keys(fallbackMap || {}), ...Object.keys(primaryMap || {})]);
  const result = {};
  regions.forEach((region) => {
    result[region] = mergeTrendPointLists(primaryMap?.[region], fallbackMap?.[region]);
  });
  return result;
}

// When one product has exactly one recognized Android version, a legacy
// no-Android sibling is only an incomplete metadata copy. Do not simply delete
// it: newer dates may temporarily land on that sibling. Merge its missing/fresher
// points into the recognized Android line, preferring real collection points
// over carry-forward values. This fixes UVIP/SVIP showing "today not collected"
// even though Android 10 was collected on the same day.
function mergeUnknownAndroidDuplicateLines(seriesList) {
  const groups = new Map();
  stableArray(seriesList).forEach((series) => {
    const key = `${productLineFingerprintWithoutAndroid(series)}||${seriesPurchaseMode(series)}`;
    if (!groups.has(key)) groups.set(key, []);
    groups.get(key).push(series);
  });
  const merged = [];
  groups.forEach((items) => {
    const known = items.filter((series) => Boolean(androidVersionText(series)));
    const unknown = items.filter((series) => !androidVersionText(series));
    if (known.length === 1 && unknown.length) {
      const primary = { ...known[0] };
      unknown.forEach((fallback) => {
        primary.points = mergeTrendPointLists(primary.points, fallback.points);
        primary.regional_points = mergeRegionalTrendMaps(primary.regional_points, fallback.regional_points);
      });
      Object.assign(primary, buildTrendStatsFromPoints(primary.points));
      primary.legacy_unknown_android_points_merged = true;
      primary.legacy_unknown_android_source_count = unknown.length;
      merged.push(primary);
      return;
    }
    merged.push(...items);
  });
  return merged.sort(sortSeriesForDecision);
}

function comparePairedDefaultSeries(a, b, androidDisplayMode) {
  if (androidDisplayMode === "expanded") {
    const aHasAndroid = Boolean(androidVersionText(a));
    const bHasAndroid = Boolean(androidVersionText(b));
    if (aHasAndroid !== bHasAndroid) return aHasAndroid ? -1 : 1;
  }
  const aUnknownConfig = /android_未知安卓/i.test(String(a?.ug_config_id || ""));
  const bUnknownConfig = /android_未知安卓/i.test(String(b?.ug_config_id || ""));
  if (aUnknownConfig !== bUnknownConfig) return aUnknownConfig ? 1 : -1;
  return sortSeriesForDecision(a, b);
}
...
function mergeSeriesForProductLineDisplay(items, groupIndex = 0) {
  ...
  return {
    ...
    purchase_mode: seriesPurchaseMode(base),
    ...
    return raw.flatMap((series) => {
      ...
      if (children.length) {
        return children.map((child) => {
          ...
          // If the product genuinely has only one Android version (UVIP/SVIP),
          // the parent merged line and the Android child describe the same SKU.
          // A current-day point may land on the parent while the child still has
          // yesterday's carry-forward point. Merge the parent into that sole
          // child, preferring real collection points over carry-forward values.
          const singleVersionFallback = children.length === 1;
          const points = singleVersionFallback
            ? mergeTrendPointLists(child.points, series.points)
            : child.points;
          const regionalPoints = singleVersionFallback
            ? mergeRegionalTrendMaps(child.regional_points, series.regional_points)
          ...
          const expanded = {
            ...
            purchase_mode: child.purchase_mode || series.purchase_mode || seriesPurchaseMode(series),
            points,
            regional_points: regionalPoints,
            single_android_parent_points_merged: singleVersionFallback,
          };
          return { ...expanded, ...buildTrendStatsFromPoints(points) };
        });
      }
      return [{
        ...series,
...
function buildCompositeIndexRows(seriesList) {
  // The composite index must use the exact same canonical product-line layer as
  // chart two.  Raw price_trends series can contain Android-known/unknown or
  // historical duplicate variants for one product.  Averaging raw rows makes a
  // new snapshot of such a duplicate look like a price change even when every
  // displayed product line is stable.
  const indexRows = [];
  ...
  seriesList.forEach((series) => {
    stableArray(series.points).forEach((point) => {
      ...
      const input = {
        ...
        series_id: getSeriesKey(series),
        product_line: getSeriesDisplayName(series),
        price_source: point.price_source || null,
      };
      allBaseCandidates.push(input);
      if (series.platform === "UgPhone") ugBaseCandidates.push(input);
    });
  });
  ...
  [...dates].sort((a, b) => sortDateLabel(a).localeCompare(sortDateLabel(b))).forEach((date) => {
    ...
    allPlatforms.forEach((platform) => {
      ...
      row[platform] = values.length ? values.reduce((sum, value) => sum + value.index, 0) / values.length : null;
      row[`${platform}__count`] = values.length;
      // Expose the canonical inputs in the tooltip. This makes future index/
      // product-line mismatches auditable instead of silently averaging hidden rows.
      row[`${platform}__inputs`] = values.map((value) => ({
...
function normalizedTooltipItems(payload) {
  const bestByLogicalKey = new Map();
  stableArray(payload).forEach((item, index) => {
    const visualKey = String(item?.dataKey || item?.value || `tooltip-${index}`);
    if (isOverlapPatternDataKey(visualKey)) return;
    const logicalKey = logicalChartDataKey(visualKey);
    const rawLabel = String(item?.name || item?.payload?.[`${visualKey}__label`] || logicalKey);
    const score = (String(item?.type || "").toLowerCase() === "line" ? 100 : 0)
      + (!isRawLegendLabel(rawLabel) ? 20 : 0)
      + (item?.payload?.[`${visualKey}__label`] ? 10 : 0)
      + (Number.isFinite(Number(item?.value)) ? 1 : 0);
    const candidate = { ...item, visualKey, logicalKey, rawLabel, index, score };
    const previous = bestByLogicalKey.get(logicalKey);
    if (!previous || candidate.score > previous.score) bestByLogicalKey.set(logicalKey, candidate);
  });
  return [...bestByLogicalKey.values()].sort((a, b) => a.index - b.index);
}

function TrendsTooltip({ active, payload, label }) {
  if (!active || !payload?.length) return null;
  const items = normalizedTooltipItems(payload);
  if (!items.length) return null;
  return (
    <div className="relative z-[9999] max-w-[300px] rounded-lg border border-line bg-white p-3 text-xs shadow-2xl">
      <div className="mb-2 font-bold">日期：{label}</div>
      <div className="grid gap-2">
        {items.map((item) => {
          const source = item.payload?.[`${item.visualKey}__source`];
          const carriedFromDate = item.payload?.[`${item.visualKey}__carried_from_date`];
          const sourceCollectionDate = item.payload?.[`${item.visualKey}__source_collection_date`];
          const count = item.payload?.[`${item.visualKey}__count`];
          const actual = item.payload?.[`${item.visualKey}__actual_value`];
          const rawValue = Number.isFinite(Number(actual)) ? actual : item.value;
          const labelText = item.name || item.payload?.[`${item.visualKey}__label`] || item.rawLabel || item.logicalKey;
          const isCompositeIndex = String(labelText).includes("综合指数") || allPlatforms.includes(item.logicalKey);
          return (
            <div key={item.logicalKey} style={{ color: item.color }}>
              <div className="font-semibold">{labelText}</div>
              <div>{isCompositeIndex ? `指数：${indexValue(rawValue)}` : `价格：${money(rawValue)}`}</div>
              {isCompositeIndex && item.payload?.__index_base_date ? <div>指数基准：{item.payload.__index_base_date} {item.payload.__index_base_mode || "UgPhone均价"}=100</div> : null}
              {isCompositeIndex && count !== undefined ? <div>纳入产品线：{count} 条</div> : null}
...
function isRawLegendLabel(value) {
  const text = String(value || "").trim();
  return !text || /^series[_:]/i.test(text) || text.includes("__") || text.includes("product_line_");
}

function normalizedLegendItems(payload) {
  const bestByDataKey = new Map();
  stableArray(payload).forEach((item, index) => {
    const dataKey = String(item?.dataKey || item?.value || `legend-${index}`);
    if (isOverlapPatternDataKey(dataKey)) return;
    const rawLabel = String(item?.value || item?.name || dataKey);
    const score = (String(item?.type || "").toLowerCase() === "line" ? 100 : 0)
      + (!isRawLegendLabel(rawLabel) ? 10 : 0);
    const candidate = { ...item, dataKey, legendLabel: rawLabel, index, score };
    const previous = bestByDataKey.get(dataKey);
    if (!previous || candidate.score > previous.score) bestByDataKey.set(dataKey, candidate);
  });
  return [...bestByDataKey.values()].sort((a, b) => a.index - b.index);
}

function legendPlatformFromItem(item) {
  const text = [item?.dataKey, item?.legendLabel, item?.value, item?.name]
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
  return allPlatforms.find((platform) => text.includes(platform.toLowerCase())) || null;
}

function legendColorFromItem(item) {
  // Recharts may omit color for a custom legend or inherit a white fill from an
  // Area payload. Prefer the known platform color whenever the series belongs to
  // a monitored platform so every button keeps a visible, stable swatch.
  const platform = legendPlatformFromItem(item);
  if (platform) return platformColors[platform];
  const provided = String(item?.color || "").trim();
  return /^#(?:[0-9a-f]{3}|[0-9a-f]{6})$/i.test(provided) ? provided : "#94a3b8";
}

function InteractiveChartLegend({ payload, focusedDataKey, onFocusChange, maxHeightClass = "max-h-20", normalizeDataKey = logicalChartDataKey }) {
  const items = normalizedLegendItems(payload);
  if (!items.length) return null;
  return (
    <div className={`mx-auto mt-2 ${maxHeightClass} overflow-y-auto rounded-md border border-line bg-white/90 px-3 py-2 text-xs shadow-sm`}>
      <div className="flex flex-wrap justify-center gap-x-2 gap-y-1.5">
        {items.map((item) => {
          const visualDataKey = item.dataKey;
          const dataKey = normalizeDataKey(visualDataKey);
          const highlighted = !focusedDataKey || focusedDataKey === dataKey;
          const selected = focusedDataKey === dataKey;
          const swatchColor = legendColorFromItem(item);
          return (
            <button
              className={`inline-flex max-w-[280px] items-center gap-1.5 rounded-md px-1.5 py-1 text-left transition-all ${highlighted ? "text-slate-700" : "opacity-35"} ${selected ? "bg-slate-100 ring-1 ring-slate-300" : "hover:bg-slate-50"}`}
              key={visualDataKey}
              title={selected ? "再次点击恢复全部曲线" : "点击突出该曲线并显示面积填充"}
              type="button"
              onClick={() => onFocusChange?.(selected ? null : dataKey)}
            >
              <span aria-hidden="true" className="h-3 w-3 shrink-0 rounded-[3px] ring-1 ring-black/10" style={{ backgroundColor: swatchColor }} />
...
function seriesPairingProductModels(series) {
  const products = new Set();
  const addProduct = (value) => {
    const normalized = cleanProductModelForProductLine(value || "");
    if (productModels.includes(normalized)) products.add(normalized);
  };
  const addProductsFromEncodedValue = (value) => {
    const text = String(value || "");
    if (!text) return;
    if (text.startsWith("ug_product_model::")) addProduct(text.replace("ug_product_model::", ""));
    const upper = text.toUpperCase();
    productModels.forEach((product) => {
      const pattern = new RegExp(`(^|[^A-Z0-9])${product}([^A-Z0-9]|$)`, "i");
      if (pattern.test(upper)) addProduct(product);
    });
  };

  addProduct(series?.ug_product_model);
  ...
  [
    series?.ug_config_id,
    ...stableArray(series?.ug_config_ids),
    ...stableArray(series?.source_series_ids),
  ].forEach(addProductsFromEncodedValue);

  if (series?.platform === "UgPhone") addProduct(series?.product_model);
  return products;
}
...
function seriesMatchesConfig(series, configId) {
  ...
  if (target.startsWith("ug_product_model::")) {
    const expected = cleanProductModelForProductLine(target.replace("ug_product_model::", ""));
    return seriesPairingProductModels(series).has(expected);
  }
...
function TrendsPage({ data }) {
  ...
  const compositePointRegistryRef = useRef(new Map());
  const detailPointRegistryRef = useRef(new Map());
  ...
  const indexRawSeries = useMemo(() => {
    return stableArray(payload.series).filter((series) => {
      if (String(series.duration_bucket) !== String(indexBucket)) return false;
      if (platformSupportsPurchaseMode(series.platform) && seriesPurchaseMode(series) !== "subscription") return false;
  ...
  // Figure one and figure two now share the same merged Android/region product-line
  // reduction. This prevents a duplicate raw snapshot from contaminating only the
  // latest composite-index day.
  const indexSeries = useMemo(
    () => makeProductLineCandidateSeries(makeDisplayTrendSeries(indexRawSeries, "merged")),
    [indexRawSeries],
  );
  const compositeRawRows = useMemo(() => buildCompositeIndexRows(indexSeries), [indexSeries]);
  const compositeRenderModel = useMemo(
    () => buildOverlapPatternRenderModel(compositeRawRows, allPlatforms, "index", { nearValueEpsilon: 2.0 }),
    [compositeRawRows],
  );
  const compositeRows = compositeRenderModel.rows;
  const compositeOverlapPatterns = compositeRenderModel.overlapPatterns;
  const compositeProximityPatterns = compositeRenderModel.proximityPatterns || [];
  const indexDateTicks = useMemo(() => buildChartDateTicks(compositeRows, 8), [compositeRows]);
  ...
  const baseRawDetailSeries = useMemo(() => {
    ...
    const filterRawSeries = (ignorePairedConfig = false) => stableArray(payload.series).filter((series) => {
      ...
      if (platformSupportsPurchaseMode(series.platform) && seriesPurchaseMode(series) !== purchaseMode) return false;
      if (focusedBrand) {
    ...
    // Non-subscription UgPhone and VSPhone lines are reconstructed from strict
    // products history rather than the subscription-first quality workbook.
    // Add a mode-aware fallback only for a platform missing from the paired
    // result; this prevents a valid one-time line from disappearing merely
    // because another platform already made the result non-empty.
    if (!focusedBrand && purchaseMode === "non_subscription") {
      const targetProduct = effectiveConfigId.startsWith("ug_product_model::")
        ? cleanProductModelForProductLine(effectiveConfigId.replace("ug_product_model::", ""))
        : "";
      const presentPlatforms = new Set(rawSeries.map((series) => series.platform));
      const modeFallback = stableArray(payload.series).filter((series) => {
        if (!platformSupportsPurchaseMode(series.platform)) return false;
        if (presentPlatforms.has(series.platform)) return false;
        if (String(series.duration_bucket) !== String(detailBucket)) return false;
        if (seriesPurchaseMode(series) !== "non_subscription") return false;
        if (series.platform === "UgPhone") {
          return !targetProduct || seriesPairingProductModels(series).has(targetProduct);
        }
        return seriesMatchesConfig(series, effectiveConfigId)
          || stableArray(series.ug_config_ids).includes("history_non_subscription");
      });
      if (modeFallback.length) return [...rawSeries, ...modeFallback];
    }
    ...
    return rawSeries;
  }, [payload.series, detailBucket, effectiveConfigId, platformFilter, levelMode, brandFocus, purchaseMode]);
  ...
  const regionScopeKey = [brandFocus, detailBucket, effectiveConfigId, platformFilterKey, levelMode, androidDisplayMode, purchaseMode].join("||");
  ...
  const candidateSeries = useMemo(() => {
    const q = search.trim().toLowerCase();
    const groupedDisplaySeries = makeProductLineCandidateSeries(baseDisplaySeries);
    // In Android-expanded mode, collapse only legacy "Android unknown" duplicates
    // when an identical explicitly-versioned product line exists. This keeps the
    // UVIP paired group on its real Android 10 line while preserving products that
    // genuinely do not expose an Android version at all.
    const displaySeries = androidDisplayMode === "expanded"
      ? mergeUnknownAndroidDuplicateLines(groupedDisplaySeries)
      : groupedDisplaySeries;
    return displaySeries.filter((series) => {
      // UgPhone has retired the 15-day purchase duration for a long time.
      // Hide only that retired UgPhone duration in Figure 2.  Do not infer
      // retirement from one missed scrape for any other product/duration.
      if (series.platform === "UgPhone" && Number(series.duration_bucket) === 15) return false;
      if (platformSupportsPurchaseMode(series.platform) && seriesPurchaseMode(series) !== purchaseMode) return false;
  ...
  }, [baseDisplaySeries, changeMode, search, purchaseMode, androidDisplayMode]);

  useEffect(() => {
    setSelectedSeriesIds(null);
    setDetailFocus(null);
  }, [detailBucket, effectiveConfigId, platformFilter, levelMode, changeMode, androidDisplayMode, purchaseMode, search, brandFocus]);

  // Machine-room selection is orthogonal to product-line selection.  Changing
  // between merged/all/specific regions must never reset the package buttons.
  useEffect(() => {
    setDetailFocus(null);
  }, [selectedRegionKey]);
  ...
  const defaultDetailSeries = useMemo(() => {
    ...
    candidateSeries.forEach((series) => {
      const current = byPlatform.get(series.platform);
      if (!current || comparePairedDefaultSeries(series, current, androidDisplayMode) < 0) {
        byPlatform.set(series.platform, series);
      }
    });
    return allPlatforms.map((platform) => byPlatform.get(platform)).filter(Boolean);
  }, [candidateSeries, brandFocus, androidDisplayMode]);
  ...
  const detailRawRows = useMemo(() => makeWideChartRows(chartSeries), [chartSeries]);
  const detailRenderModel = useMemo(() => buildOverlapPatternRenderModel(detailRawRows, chartSeries.map(getSeriesChartKey), "detail"), [detailRawRows, chartSeries]);
  const detailRows = detailRenderModel.rows;
  const detailOverlapPatterns = detailRenderModel.overlapPatterns;
  const detailProximityPatterns = detailRenderModel.proximityPatterns || [];
  const detailDateTicks = useMemo(() => buildChartDateTicks(detailRows, 8), [detailRows]);


  useEffect(() => {
    compositePointRegistryRef.current = new Map();
  }, [compositeRows.length, indexFocus, indexBucket]);

  useEffect(() => {
    detailPointRegistryRef.current = new Map();
  }, [detailRows.length, detailFocus, detailBucket, chartSeries.length]);
  ...
  return (
    <div className="space-y-6">
      <InfoCard title="本页怎么看">
        本页分成两个图：图一用“2026-04-28 UgPhone均价=100”作为全局基准，观察四个平台的整体价格水平和变化方向；图二既可以按 UgPhone 配对组查看四家对应产品，也可以直接选择某个品牌展示该品牌全部产品线，并可用“机房展示方式”在“合并所有机房”和具体机房多选之间切换。图表横轴按自然日连续展示，但数据按采集日处理：只有该平台当天没有采集时才沿用上一采集日价格并标记 carry_forward；若平台当天已采集但某产品/购买周期未出现，则视为采集缺口或停售/不可用，折线会断开，不会用旧价伪装成当天新价格。若某产品在此前从未有过有效价格，则折线从其首个有效采集日开始，不会绘制为零值。核心趋势默认排除新客价、试用价、渠道秒杀价和多设备组合包；同产品多机房价格按“多数机房价格优先，数量持平选低价”。当多条线在连续日期内真实价格完全重合时，图中只在该真实重合区间显示随线段变化的多色渐变轨迹；悬浮窗仍显示真实价格。点击某条图例后，涉及该产品的重合区间会完全显示为该产品颜色。注意：7天档中 LDCloud 使用8天套餐参与比较，因为 LDCloud 无7天购买时限。
      </InfoCard>
      ...
      <section className="panel p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">价格指数定义：选定消费时长下，以 2026-04-28 的 UgPhone 产品均价作为 100；其他平台/日期的指数 = 当日该平台纳入产品线均价 ÷ 4月28日 UgPhone基准均价 × 100。图一与图二共用同一套“合并 Android 版本与机房后的核心产品线”口径，因此图二对应产品线价格不变时，图一不会因隐藏重复序列而波动。横轴按自然日展示；只有平台当天未采集时才显示上一采集日价格并标记 carry_forward。平台已采集但产品或购买周期缺失时折线断开，不再沿用旧价；此前从未有有效价格的产品线从首个有效日期开始绘制，不会用零值占位。点击下方图例可突出曲线并显示半透明面积填充；涉及重合价格的区间会完全切换为当前选中产品的颜色。</p>
          </div>
        ...
        <div className="mt-4 h-80">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={compositeRows} margin={{ top: 12, right: 24, left: 8, bottom: 10 }}>
              <defs>
                {allPlatforms.map((platform) => (
                  <linearGradient id={`index-area-${platform}`} key={platform} x1="0" x2="0" y1="0" y2="1">
                    <stop offset="0%" stopColor={platformColors[platform]} stopOpacity={0.24} />
                    <stop offset="100%" stopColor={platformColors[platform]} stopOpacity={0.015} />
                  </linearGradient>
                ))}
                {compositeOverlapPatterns.map((pattern) => {
                  const colors = pattern.sourceKeys.map((key) => platformColors[key] || "#64748b");
                  return (
                    <linearGradient id={pattern.gradientId} key={pattern.gradientId} x1="0%" y1="0%" x2="100%" y2="0%">
                      {buildOverlapGradientStops(colors).map((stop, index) => (
                        <stop key={`${pattern.gradientId}-${index}`} offset={stop.offset} stopColor={stop.color} stopOpacity={1} />
                      ))}
                    </linearGradient>
                  );
                })}
              ...
              />
              <YAxis domain={["auto", "auto"]} width={48} tickLine={false} axisLine={{ stroke: "#94a3b8" }} tick={{ fill: "#64748b", fontSize: 12 }} />
              <Tooltip content={<TrendsTooltip />} wrapperStyle={{ zIndex: 9999, pointerEvents: "none" }} />
              <Legend verticalAlign="bottom" align="center" height={52} content={<InteractiveChartLegend focusedDataKey={indexFocus} onFocusChange={setIndexFocus} maxHeightClass="max-h-14" normalizeDataKey={logicalChartDataKey} />} />
              {allPlatforms.map((platform) => {
                const selected = indexFocus === platform;
                return (
                  <Area
                    key={`area-${platform}-${indexFocus || "all"}`}
                    type="monotone"
                    dataKey={visualChartDataKey(platform)}
                    fill={`url(#index-area-${platform})`}
              ...
              {allPlatforms.map((platform) => {
                const selected = indexFocus === platform;
                const dimmed = indexFocus && !selected;
                return (
                  <Line
                    key={`line-${platform}-${indexFocus || "all"}-${indexBucket}`}
                    type="monotone"
                    dataKey={visualChartDataKey(platform)}
                    name={`${platform} 综合指数`}
                    stroke={platformColors[platform]}
                    strokeOpacity={dimmed ? 0.2 : 1}
                    strokeWidth={selected ? 4.8 : 3.7}
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    dot={false}
                    activeDot={false}
                    connectNulls
                    shape={(shapeProps) => (
                      <OverlapAwareLineShape
                        {...shapeProps}
                        logicalKey={platform}
                        patterns={compositeOverlapPatterns}
                        proximityPatterns={compositeProximityPatterns}
                        pointRegistryRef={compositePointRegistryRef}
                        focusedDataKey={indexFocus}
                        colorForKey={(key) => platformColors[key] || "#64748b"}
                        revealKey={`index:${indexBucket}:${indexFocus || "all"}:${compositeRows.length}:${compositeRows.at(-1)?.date || ""}`}
                      />
                    )}
      ...
      <section className="panel p-5">
        ...
        <p className="mt-1 text-sm text-muted">UgPhone 与 VSPhone 的价格方式默认使用订阅价格；历史数据在 purchase_mode 字段出现前也按自动续费开启处理。切换为“非订阅价格”后，两者都会使用关闭自动续费时的单品卡片价格，Redfinger 与 LDCloud 仍使用其标准价格。默认展示 “UgPhone UVIP” 配对组下四个平台各一条核心产品线；你也可以切换为某个品牌全集，直接查看该品牌所有产品线。机房展示方式会随当前查看范围变化：切换“查看方式”后默认回到“合并所有机房”；只有手动勾选具体机房或点击“全选”时，才会按“产品线 × 机房”分别绘制折线。按 UgPhone 配对组会锁定为合并所有机房。清空后不会自动重新勾选产品线。</p>
        <div className="mt-5 grid gap-x-4 gap-y-3 md:grid-cols-2 xl:grid-cols-12">
          <SelectFilter className="xl:col-span-3" label="购买天数" value={detailBucket} onChange={setDetailBucket}>{[...coreBuckets, "other"].map((item) => <option key={item} value={item}>{bucketLabel(item)}</option>)}</SelectFilter>
          <SelectFilter className="xl:col-span-3" label="查看方式" value={brandFocus} onChange={(value) => { setBrandFocus(value); resetRegionDisplayToMerged(); }}>
          ...
          <label className="grid gap-1 text-xs font-semibold text-muted xl:col-span-3">
            UgPhone 配对配置
          ...
          <SelectFilter className="xl:col-span-3" label="价格变化" value={changeMode} onChange={setChangeMode}><option value="all">全部</option><option value="changed">只看有变化</option><option value="unchanged">只看未变化</option></SelectFilter>
          <SelectFilter className="xl:col-span-3" label="安卓版本展示方式" value={androidDisplayMode} onChange={changeAndroidDisplayMode}><option value="merged">合并安卓版本（默认）</option><option value="expanded">展开安卓版本</option></SelectFilter>
          <div className="grid gap-1 text-xs font-semibold text-muted xl:col-span-3">
            UgPhone 价格方式
            <div className="grid grid-cols-2 rounded-md border border-line bg-white p-1 text-sm font-semibold">
              <button
                className={`rounded px-2 py-1.5 ${purchaseMode === "subscription" ? "bg-red-50 text-red-700 shadow-sm" : "text-muted"}`}
                onClick={() => { setPurchaseMode("subscription"); setSelectedSeriesIds(null); setDetailFocus(null); }}
                type="button"
              >订阅价格</button>
              <button
                className={`rounded px-2 py-1.5 ${purchaseMode === "non_subscription" ? "bg-slate-100 text-ink shadow-sm" : "text-muted"}`}
                onClick={() => { setPurchaseMode("non_subscription"); setSelectedSeriesIds(null); setDetailFocus(null); }}
                type="button"
              >非订阅价格</button>
            </div>
          </div>
          <div className="grid gap-1 text-xs font-semibold text-muted md:col-span-2 xl:col-span-6">
            机房展示方式
            <div className={`flex flex-wrap items-center gap-2 rounded-md border border-line bg-white px-3 py-2 ${regionControlLocked ? "bg-slate-100 text-slate-400" : ""}`}>
              <div className="order-2 flex flex-wrap gap-1.5 sm:order-1 sm:shrink-0">
                <button
              ...
              <div className="order-1 flex min-w-[220px] flex-1 flex-wrap items-center gap-2 sm:order-2">
                ...
                {regionControlLocked ? (
                  <span className="text-xs font-normal text-muted">按 UgPhone 配对组锁定。</span>
                  ...
                  <span className="text-xs font-normal text-muted">当前查看方式没有可拆分的机房历史数据。</span>
                )}
              </div>
            </div>
            <span className="text-[11px] font-normal text-muted">红色为合并选项；选择任一具体机房时会自动取消合并。</span>
          </div>
          <SearchFilter label="搜索" value={search} onChange={setSearch} placeholder="搜索平台、产品、配置、机房" className="xl:col-span-6" />
          <div className="grid gap-1 text-xs font-semibold text-muted md:col-span-2 xl:col-span-6">
            平台
        ...
        <div className="mt-4 h-[30rem]">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={detailRows} margin={{ top: 12, right: 24, left: 8, bottom: 12 }}>
              <defs>
                {chartSeries.map((series, index) => {
                  const key = getSeriesKey(series);
                  const color = chartSeriesColorMap.get(key) || platformSeriesColor(series.platform, index);
                  return (
                    <linearGradient id={`detail-area-${index}`} key={`detail-gradient-${key}`} x1="0" x2="0" y1="0" y2="1">
                      <stop offset="0%" stopColor={color} stopOpacity={0.22} />
                      <stop offset="100%" stopColor={color} stopOpacity={0.015} />
                    </linearGradient>
                  );
                })}
                {detailOverlapPatterns.map((pattern) => {
                  const colors = pattern.sourceKeys.map((chartKey) => {
                    const sourceSeries = chartSeries.find((series) => getSeriesChartKey(series) === chartKey);
                    if (!sourceSeries) return "#64748b";
                    return chartSeriesColorMap.get(getSeriesKey(sourceSeries)) || "#64748b";
                  });
                  return (
                    <linearGradient id={pattern.gradientId} key={pattern.gradientId} x1="0%" y1="0%" x2="100%" y2="0%">
                      {buildOverlapGradientStops(colors).map((stop, index) => (
              ...
              <XAxis
                ...
                tick={{ fill: "#64748b", fontSize: 12 }}
              />
              <YAxis width={52} tickLine={false} axisLine={{ stroke: "#94a3b8" }} tick={{ fill: "#64748b", fontSize: 12 }} />
              <Tooltip content={<TrendsTooltip />} wrapperStyle={{ zIndex: 9999, pointerEvents: "none" }} />
              <Legend verticalAlign="bottom" align="center" height={64} content={<InteractiveChartLegend focusedDataKey={detailFocus} onFocusChange={setDetailFocus} maxHeightClass="max-h-16" normalizeDataKey={logicalChartDataKey} />} />
              {chartSeries.map((series, index) => {
                const key = getSeriesKey(series);
                const chartKey = getSeriesChartKey(series);
                const selected = detailFocus === chartKey;
                return (
                  <Area
                    key={`area-${chartKey}-${detailFocus || "all"}`}
                    type="monotone"
                    dataKey={visualChartDataKey(chartKey)}
                    fill={`url(#detail-area-${index})`}
                    fillOpacity={selected ? 1 : 0}
                    stroke="none"
                    connectNulls
                    isAnimationActive
              ...
              {chartSeries.map((series, index) => {
                ...
                const color = chartSeriesColorMap.get(key) || platformSeriesColor(series.platform, index);
                const selected = detailFocus === chartKey;
                const dimmed = detailFocus && !selected;
                return (
                  <Line
                    key={`line-${chartKey}-${detailFocus || "all"}-${detailBucket}`}
                    type="monotone"
                    dataKey={visualChartDataKey(chartKey)}
                    name={getSeriesDisplayName(series)}
                    stroke={color}
                    strokeOpacity={dimmed ? 0.2 : 1}
                    strokeWidth={selected ? 4.8 : 3.6}
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    dot={false}
                    activeDot={false}
                    ...
                    shape={(shapeProps) => (
                      <OverlapAwareLineShape
                        {...shapeProps}
                        logicalKey={chartKey}
                        patterns={detailOverlapPatterns}
                        proximityPatterns={detailProximityPatterns}
                        pointRegistryRef={detailPointRegistryRef}
                        focusedDataKey={detailFocus}
                        colorForKey={(focusedKey) => {
                          const selectedSeries = chartSeries.find((item) => getSeriesChartKey(item) === focusedKey);
                          return selectedSeries ? (chartSeriesColorMap.get(getSeriesKey(selectedSeries)) || "#64748b") : "#64748b";
                        }}
                        revealKey={`detail:${detailBucket}:${detailFocus || "all"}:${chartSeries.length}:${detailRows.length}:${detailRows.at(-1)?.date || ""}`}
                      />
                    )}
                    isAnimationActive={false}
                  />
      ...
      <section className="panel overflow-hidden">
        <div className="overflow-x-auto">
          <table className="data-table w-full min-w-[1240px] text-left text-sm">
            <thead className="bg-slate-50 text-xs text-muted">
              <tr><th>平台</th><th>配置</th><th>购买天数</th><th>历史 baseline</th><th>上次采集价</th><th>本次 products.csv 当前价</th><th>变化金额</th><th>变化比例</th><th>商品文本变化</th><th>reason_code</th><th>alert_level</th></tr>
            </thead>
            <tbody className="divide-y divide-line">
...
export default function App() {
  ...
  return (
    <AppShell page={page} setPage={setPage} lang={lang} setLang={setLang} onReload={loadData} isReloading={isReloading} dataMeta={data.meta}>
      {content}
    </AppShell>
  );
}
